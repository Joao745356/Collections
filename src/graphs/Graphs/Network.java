...
import graphs.Interfaces.NetworkADT;
import list.arrayLists.ArrayUnorderedList;
import queue.LinkedQueue;
import stack.LinkedStack;
import trees.binary.heap.ArrayHeap;
import trees.binary.heap.LinkedHeap;
import trees.binary.interfaces.HeapADT;
 ...
 */
public class Network<T> extends Graph<T> implements NetworkADT<T> { //weighted Graph
    ...
    private Iterator<T> iteratorBFS(int startIndex) {
        ...
        if (super.indexIsValid(startIndex)) {
            ...
            while (!traversalQueue.isEmpty()) {
                try {
                    Integer x = (Integer) traversalQueue.dequeue();
    ...
    private Iterator<T> iteratorDFS(int startIndex) {
        LinkedStack<Integer> traversalStack = new LinkedStack<>(); //store indexes
        ArrayUnorderedList<T> resultList = new ArrayUnorderedList<>();
        boolean[] visited = new boolean[super.size()];
        if (super.indexIsValid(startIndex)) {
            int i;
    ...
    private Iterator<T> iteratorShortestPath(int startIndex, int targetIndex) {
        ...
        if (this.indexIsValid(startIndex) && this.indexIsValid(targetIndex)) {
            ...
            while (it.hasNext()) {
                templist.addToRear(this.vertices[(Integer) it.next()]);
            }
    ...
    protected Iterator<Integer> iteratorShortestPathIndices(int startIndex, int targetIndex) {
        ...
        int[] predecessor = new int[this.numVertices];   //para cada vértice tens o verti anterior
        HeapADT<Double> traversalMinHeap = new LinkedHeap<>(); //pesos das arestas no grafo
        ArrayUnorderedList<Integer> resultList = new ArrayUnorderedList<>(); //lista com os nodes
        LinkedStack<Integer> stack = new LinkedStack(); //para fazer a travessia
        int[] pathIndex = new int[this.numVertices]; // array dos indices mais "baratos"
        double[] pathWeight = new double[this.numVertices]; // custo de ir de vértice em vértice
        ...
        if (this.indexIsValid(startIndex) && this.indexIsValid(targetIndex) && startIndex != targetIndex && !this.isEmpty()) {
            ...
            do {
                try {
                    ...
                    weight = (Double) traversalMinHeap.removeMin();
                ...
                } catch (EmptyCollectionException var15) {
                }
            ...
            while (!stack.isEmpty()) {
                ...
                } catch (EmptyCollectionException var14) {
                }
            }
    ...
    private double shortestPathWeight(int startIndex, int targetIndex) {
        ...
        if (this.indexIsValid(startIndex) && this.indexIsValid(targetIndex)) {
            ...
            if (!it.hasNext()) {
                ...
                int index2;
                for(int index1 = (Integer) it.next(); it.hasNext(); index1 = index2) {
